...
::
   ...
   <domain>
     ...
     <launchSecurity type='tdx'>
       ...
       <quoteGenerationSocket type='vsock' cid='xxx' port='xxx'/>
     ...
   </domain>
...
``quoteGenerationSocket``
   ...
   Service(QGS) daemon socket address configuration. User in TD guest cannot
   get TD quoting for attestation if QGS socket address is not provided. This
   subelement includes a ``type`` attribute to determine the socket type chosed,
   it can be "inet", "unix", "vsock" or "fd". Different ``type`` further
   determines other optional atributes supported.
